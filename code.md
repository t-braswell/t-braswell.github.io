---
title: Code
permalink: /code
menus: header
---
{% for item in site.data.navigation.header %} \- [{{ item.title }}]({{site.baseurl}}{{ item.url }}) {% endfor %}


# [**Julia Set Visualizer:**](https://github.com/t-braswell/GraphingJuliaSets){:target="_blank"} 
a JDK-based GUI app for generating Julia sets for particular values of z=x+iy


# [**Zmodp Algorithms:**](https://github.com/t-braswell/zmodp-algorithms){:target="_blank"} 
A collection of algorithms for a introductory ring theory class, largely for the sake of homework checking.

# [**SLiMski themes**](https://github.com/t-braswell/slimski_themes){:target="_blank"} 
A fork of a previous collection of SLiM themes (display manager for pre-systemd linux), modified for use in antiX Linux’s display manager fork SLiMski
